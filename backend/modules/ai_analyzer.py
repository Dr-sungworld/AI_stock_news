...
def _call_gemini_api(prompt: str) -> Optional[str]:
    ...
    try:
        ...
        # if response is not None: # response might be unbound if requests.post fails
        #      logger.error(f"Response: {response.text}")
        return None

def analyze_news(news_item: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """
    Analyzes news for importance, themes, and historical context.
    
    Args:
        news_item: Dictionary containing news details (title, snippet, link, etc.)
